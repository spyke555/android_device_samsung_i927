...
int KXTFSensor::setInitialState()
{
    ...
    if (mHasPendingEvent) {
        ALOGD(TAG "%s: %d %d %d -> %f %f %f", __FUNCTION__,
                absinfo_x.value, absinfo_y.value, absinfo_z.value,
...
int KXTFSensor::enable(int32_t, int en)
{
    ...
    if (flags != mEnabled) {
        ...
        ALOGE(TAG "%s(%d) failed, err %d", __FUNCTION__, en, errno);
        return -1;
    }
    return 0;
}
...
again:
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        if (type == EV_REL) {
            ...
            ALOGE(TAG "%s: unknown event (type=%d, code=%d)",
    ...
    if (mHasPendingEvent) {
        ALOGV(TAG "%s: %d %d %d -> %f %f %f", __FUNCTION__,
                absinfo_x.value, absinfo_y.value, absinfo_z.value,
                mPendingEvent.data[0], mPendingEvent.data[1], mPendingEvent.data[2]);
    }
