...
int NctSensor::readSysFsValue(void) {
    ...
    if (fd > 0) {
        ...
        if (nr >= 0) {
            ALOGV(TAG "%s read=%d", __FUNCTION__, absinfo.value);
    ...
    if (flags != mEnabled) {
        ...
        if (mEnabled) {
            if (getFd() > 0)
                ...
                mEnabled = false;
                ALOGW(TAG "unable to enable the sensor");
                return -EIO;
            }
        }
    }
...
int NctSensor::readEvents(sensors_event_t* data, int count)
{
    ...
    while (count) {
        ALOGV(TAG "%s has pending data", __FUNCTION__);
        if (mEnabled) {
                *data++ = mPendingEvent;
                numRecords++;
        }
