...
int GyroSensor::setInitialState() {
    ...
    ALOGV(TAG "%s: data_fd=%d", __FUNCTION__, data_fd);

    if (!ioctl(data_fd, MPU_GET_DMP_CFG1, &dmp_cfg)) {
        ALOGV(TAG "ioctl success : %d", dmp_cfg);
    }
...
int GyroSensor::sec_power(int en) {
    ...
    if (flags != mEnabled) {
        ...
        ALOGV(TAG "%s(%d) %s", __FUNCTION__, en, sec_sysfs);
        ...
        ALOGE(TAG "%s failed, err %d", __FUNCTION__, errno);
        return -1;
    }
    return 0;
}
...
int GyroSensor::sec_runtest()
{
    ...
    if (mEnabled) {
        ...
        if (fd) {
            ...
            ALOGV(TAG "read %s : %d/%d/%d - rms : %d/%d/%d", pfx,
		 bias_x.value, bias_y.value, bias_z.value,
		 rms_x.value, rms_y.value, rms_z.value);
...
int GyroSensor::enable(int32_t, int en)
{
    ...
    if (flags != mEnabled) {
        ...
        ALOGV(TAG "%s(%d) %s", __FUNCTION__, en, input_sysfs_path);
        fd = open(input_sysfs_path, O_RDWR);
        if (fd >= 0) {
            char buf[2];
            int err;
...
//              sec_power(flags);
        ...
        ALOGE(TAG "%s enable failed, err %d", __FUNCTION__, errno);
        return -1;
    }
    return 0;
}
...
int GyroSensor::setDelay(int32_t handle, int64_t delay_ns)
{
    ...
    if (fd >= 0) {
        ...
        ALOGV(TAG "%s: %lld ms", __FUNCTION__, delay_ns / 1000000);
        return 0;
    }

    ALOGW(TAG "%s: %lld ms, error", __FUNCTION__, delay_ns / 1000000);
    return -1;
}
    ...
    while (count && mInputReader.readEvent(&event)) {
        ...
        if (type == EV_REL) {
            ...
            if (event->code == 0 /* EVENT_TYPE_GYRO_X */) {
                ...
                ALOGW(TAG "unhandled EV_REL code %d", event->code);
            }
        } else if (type == EV_SYN) {
            ...
            if (mEnabled) {
                if (mPendingEvent.timestamp >= mEnabledTime) {
                    *data++ = mPendingEvent;
                    numEventReceived++;
                    ALOGV(TAG "event processed %1.2f/%1.2f/%1.2f",
            ...
            ALOGE(TAG "unknown event (type=0x%x, code=0x%x)",
                    type, event->code);
        }
        mInputReader.next();
    }
